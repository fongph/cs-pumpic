...
//echo "</pre>"; die('stop');
	...
	$post_data['post_mime_type'] = $post->post_mime_type;
        ...
        /* save banners */
        if(isset($post_data['banner']) 
                and !empty($post_data['banner']) 
                and is_array($post_data['banner']) 
                and count($post_data['banner']) > 0) {
            
            delete_banners($post_ID);
            
            $banner_option = '';
            foreach($post_data['banner'] as $_banner_id) {
                
                // var_dump( $post_data['banner_option'][$_banner_id] );
                
                if(isset($post_data['banner_option'][$_banner_id]) 
                        and !empty($post_data['banner_option'][$_banner_id])) {
                    $banner_option = $post_data['banner_option'][$_banner_id];
                }
                
                if($_banner_id) {
                    set_banners($post_ID, $_banner_id, $banner_option);
                }
	...
	$format_keys = array( 'quote', 'quote_source_name', 'image', 'gallery', 'audio_embed', 'video_embed', 'banners' );
...
function delete_banners($post_id) {
    global $wpdb;
    $_post = $wpdb->get_results("SELECT b_id FROM banners WHERE post_id = ".(int)$post_id);
    if(is_array($_post) and count($_post) > 0) {
        $wpdb->delete('banners', array('post_id' => (int)$post_id ));
    }
}

function has_banners( $post_id, $banner_id ) {
    global $wpdb;
    if($post_id and $banner_id) {
        if($_post = $wpdb->get_results("SELECT * FROM banners WHERE post_id = ".(int)$post_id." AND banner_id = ".(int)$banner_id)
                and is_array($_post) 
                    and count($_post) > 0)
                return $_post[0];
    } else
...
function set_banners($post_id, $banner_id, $banner_options = null) {
    global $wpdb;
   
    if(has_banners($post_id, $banner_id)) {
//        $wpdb->delete('banners', array('post_id' => (int)$post_id ));
        $wpdb->update( 'banners', array( 'align' => ($banner_options) ? $banner_options : '' ), 
                                    array( 'post_id' => $post_id, 'banner_id' => $banner_id ) );
    } else
       $wpdb->insert( 'banners', array(   'post_id' => (int)$post_id, 
                                           'banner_id' => (int)$banner_id, 
                                           'align' => ($banner_options) ? (string)$banner_options : '' ));
