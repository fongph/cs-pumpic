...
function wp_date_format( $_format = null ) {
    if(!$_format)
        $_format = 'F j, Y';
    ...
    if ( !in_array( $_post->post_type, array( 'post', 'page', 'revision' ) ) )
		return false;
    
    $datef = _x( $_format, 'date format');
    $date = date_i18n( $datef, strtotime( $_post->post_modified ) );
    return $date;
}
