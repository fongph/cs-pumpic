...
require_once 'GoogleAnalyticsExeptions.php';
require_once 'GALog.php';
 ...
 * Auth: Edii_Shadow
 */
class GoogleAnalyticsAPI extends GALog {
        protected $_logger;
        protected $logPath = null;
        protected $logFile = 'api.log';
        protected $rotateByCopy=false;
	...
	public function __set($key, $value) {
		...
		case 'auth' :
			if (($value instanceof GoogleOauth) == false) {
				throw new GAExeptions('auth needs to be a subclass of GoogleOauth');
			}
			$this->auth = $value;
			break;
	...
	public function getWebProperties() {
		...
		if (!$this->accessToken) throw new GAExeptions('You must provide an accessToken');
	...
	protected function _query($params=array()){
		...
		if (!$this->accessToken || !$this->accountId) {
			throw new GAExeptions('You must provide the accessToken and an accountId');
		}
        ...
        /* method log */
        public function trace($msg, $category='api') {
                if(DEBUG)
                      $this -> setLog($msg, GALog::LEVEL_TRACE, $category);
        }

        public function warning($msg, $category='api') {
                if(DEBUG)
                       $this -> setLog($msg, GALog::LEVEL_WARNING, $category);
        }

        public function error($msg, $category='api') {
                if(DEBUG)
                       $this -> setLog($msg, GALog::LEVEL_ERROR, $category);
        }

        public function profile($msg, $category='api') {
                if(DEBUG)
                       $this -> setLog($msg, GALog::LEVEL_PROFILE, $category);
        }
	...
	protected function rotateFiles() {
         ...
         */
        public function setLog($msg, $level= GALog::LEVEL_INFO, $category='api') {
            
            if($this ->_logger === null)
                $this ->_logger = new GALog;
                if(DEBUG && TRACE_LEVEL>0 && $level!== GALog::LEVEL_PROFILE) {
            ...
            $this ->_logger->log($msg,$level,$category);
...
class GoogleOauthService extends GoogleOauth {
	...
	public function __construct($clientId='', $email='', $privateKey=null) {
		if (!function_exists('openssl_sign')) throw new GAExeptions('openssl extension for PHP is needed.');
	...
	public function getAccessToken($data=null) {
		...
		if (!$this->clientId || !$this->email || !$this->privateKey) {
			throw new GAExeptions('You must provide the clientId, email and a path to your private Key');
		}
	...
	protected function generateSignedJWT() {
		...
		if (!file_exists($this->privateKey) || !is_file($this->privateKey)) {
			throw new GAExeptions('Private key does not exist');
		}
		...
		if (!openssl_pkcs12_read($pkcs12, $certs, $this->password)) {
			throw new GAExeptions('Could not parse .p12 file');
		}
		if (!isset($certs['pkey'])) {
			throw new GAExeptions('Could not find private key in .p12 file');
		}
		$keyId = openssl_pkey_get_private($certs['pkey']);
		if (!openssl_sign($input, $sig, $keyId, 'sha256')) {
			throw new GAExeptions('Could not sign data');
		}
...
class GoogleOauthWeb extends GoogleOauth {
	...
	public function buildAuthUrl($params = array()) {
		...
		if (!$this->clientId || !$this->redirectUri) {
			throw new GAExeptions('You must provide the clientId and a redirectUri');
		}
	...
	public function getAccessToken($data=null) {
		...
		if (!$this->clientId || !$this->clientSecret || !$this->redirectUri) {
			throw new GAExeptions('You must provide the clientId, clientSecret and a redirectUri');
		}
	...
	public function refreshAccessToken($refreshToken) {
		...
		if (!$this->clientId || !$this->clientSecret) {
			throw new GAExeptions('You must provide the clientId and clientSecret');
		}
