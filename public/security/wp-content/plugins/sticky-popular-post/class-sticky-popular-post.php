...
class Popular_Sticky_Post {
	...
	public static function get_instance() {
		...
		if( null == self::$instance ) {
			self::$instance = new Popular_Sticky_Post();
		} // end if
	...
	private function __construct() {
		...
		add_action( 'add_meta_boxes', array( $this, 'add_popular_sticky_post_meta_box' ) );
		add_action( 'save_post', array( $this, 'save_popular_sticky_post_data' ) );
		add_action( 'wp_ajax_is_popular_sticky_post', array( $this, 'is_popular_sticky_post' ) );
	 ...
	 * @since    1.0.0
	 */
	public function add_popular_sticky_post_meta_box() {
		...
		foreach( $post_types as $post_type ) {
			...
			add_meta_box(
				'popular_post_is_sticky',
				__( 'Popular Sticky', 'popular-sticky-post' ),
				array( $this, 'popular_sticky_post_display' ),
				$post_type,
				'side',
				'low'
			);
	 ...
	 * @since    1.0.0
	 */
	public function popular_sticky_post_display( $post ) {

		// Set the nonce for security
		wp_nonce_field( plugin_basename( __FILE__ ), 'popular_sticky_post_nonce' );
	 ...
	 * @since    1.0.0
	 */
	public function save_popular_sticky_post_data( $post_id ) {

		if( isset( $_POST['popular_sticky_post_nonce'] ) 
                        and isset( $_POST['post_type'] ) 
                            and $this->user_can_save( $post_id, 'popular_sticky_post_nonce' ) ) {

			// Read the ID of the category to which we're going to stick this post
			if( isset( $_POST['popular_sticky_post_border'] ) ) {
				update_post_meta( $post_id, 'popular_sticky_post_border', esc_attr( $_POST['popular_sticky_post_border'] ) );
			} else {
				delete_post_meta( $post_id, 'popular_sticky_post_border' );
			} // end if/else
...
//	} // end add_styles
	 ...
	 * @since    1.0.0
	 */
	public function is_popular_sticky_post() {

		if( isset( $_GET['post_id'] ) ) {

			$post_id = trim ( $_GET['post_id'] );
			if( 0 == get_post_meta( $post_id, 'popular_sticky_post', true ) ) {
				die( '0' );
			} else {
				die( _e( ' - Popular Sticky Post', 'popular-sticky-post' ) );
			} // end if/else
	 ...
	 *---------------------------------------------*/
	  ...
	  * @since      1.0.0
	  */
	  public function set_popular_sticky_class( $classes ) {

	 	// If we've not set the category sticky post...
	 	if(false == $this->is_sticky_post and $this->is_sticky_post() ) {

		 	// ...append the class to the first post (or the first time this event is raised)
			$classes[] = 'popular-sticky';
	  ...
	  * @since      1.0.0
	  */
	 public function reorder_popular_posts( $posts, $query ) {
	 ...
	 * @since      1.0.0
	 */
	private function popular_has_sticky_post( $category_id ) {

		$has_sticky_post = false;

		$query = new WP_Query( 'meta_key=popular_sticky_post&meta_value=' . $category_id );
	...
	private function is_sticky_post() {
		...
		global $post;
		return get_query_var( 'cat' ) == get_post_meta( $post->ID, 'popular_sticky_post', true );
	...
	private function get_border_checkbox( $post ) {
		...
		$html = '<label for="popular_sticky_post_border">';
			$html .= '<input type="checkbox" value="1" name="popular_sticky_post_border" id="popular_sticky_post_border" ' . checked( '1', get_post_meta( $post->ID, 'popular_sticky_post_border', true ), false ) . ' />';
			$html .= __( 'Set popular stick', 'popular-sticky-post' );
		$html .= '</label>';
